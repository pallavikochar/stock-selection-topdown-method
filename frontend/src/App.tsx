...
export default function App() {
  ...
  // Snapshot the finished_at at the moment each run is triggered so we can
  // distinguish a genuinely NEW completion from the stale previous-run value.
  ...
  return (
    <div className="min-h-screen relative z-10">
      ...
      <header className="sticky top-0 z-50 border-b border-navy-800/60 bg-navy-950/90 backdrop-blur-sm">
        {/* Top row: brand + run button */}
        <div className="max-w-7xl mx-auto px-6 pt-4 pb-0 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-cyan-accent animate-pulse" />
              <span className="font-display font-700 text-xl tracking-tight text-white">Project DOIT</span>
              <span className="text-navy-700 text-sm">·</span>
              <span className="text-navy-500 text-sm font-mono">Portfolio Management</span>
            </div>
            <p className="text-xs text-navy-500 mt-0.5 ml-5">
              Top-down · Macro → Sector → Stock · Active Portfolio Management
              <span className="ml-3 font-600 text-cyan-accent">Pallavi Kochar</span>
            </p>
          </div>
          ...
          <div className="flex gap-0 mt-3">
            {TABS.map(tab => (
              <button
                ...
                className={`px-5 py-2.5 text-xs font-600 border-b-2 transition-colors ${
                  activeTab === tab.id
      ...
      {/* Run status banners (analysis tab only) */}
      {activeTab === "analysis" && isRunning && (
        <div className="sticky top-[97px] z-40 bg-amber-accent/10 border-b border-amber-accent/30 text-amber-accent text-xs font-600 text-center py-2 flex items-center justify-center gap-2">
          <span className="w-3 h-3 border-2 border-amber-accent/40 border-t-amber-accent rounded-full animate-spin" />
          Pipeline running — results will refresh automatically when complete
        </div>
      )}
      {activeTab === "analysis" && justFinished && !isRunning && (
        <div className="sticky top-[97px] z-40 bg-green-signal/10 border-b border-green-signal/30 text-green-signal text-xs font-600 text-center py-2">
